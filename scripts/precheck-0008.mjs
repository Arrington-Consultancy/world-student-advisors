...
// that 0008 is the ONLY migration this run will apply, so "apply migration
...
try {
  ...
  if (pending.length !== 1 || pending[0] !== "0008_enquiry_response_history") {
    console.error(
      "\nSTOPPING: this run is authorised to apply migration 0008 and nothing else. " +
        (pending.length === 0
          ? "Nothing is pending — 0008 may already be applied."
