...
// READ-ONLY pre-check for migration 0020 (Google Ads conversion uploads):
// the WSA-owned Qualified Lead conversion record, Tom Arrington's brief of
// 16 September 2026.
//
// Proves from live production evidence that 0020 is the ONLY migration this
...
const TAG = "0020_google_ads_conversion_uploads";
...
const allowed = [
  s => s.startsWith("CREATE TABLE `google_ads_conversion_uploads` (")
    && s.includes("CONSTRAINT `google_ads_conversion_uploads_id` PRIMARY KEY(`id`)")
    && s.includes("CONSTRAINT `google_ads_conversion_uploads_deal_action` UNIQUE(`dealId`,`conversionActionId`)"),
];
...
try {
  ...
  if (pending.length !== 1 || pending[0] !== TAG) {
    console.error(
      `\nSTOPPING: this run is authorised to apply migration 0020 and nothing else. ` +
        (pending.length === 0 ? "Nothing is pending; 0020 may already be applied." : `Pending set is [${pending.join(", ")}].`),
    );
    process.exit(1);
  }
  ...
  if (names.has("google_ads_conversion_uploads")) {
    console.error("\nSTOPPING: google_ads_conversion_uploads already exists.");
    process.exit(1);
  }
  // 0019 must be the real position, not just the journal's claim.
  if (!names.has("crm_backup_runs")) {
    console.error("\nSTOPPING: crm_backup_runs is missing, so 0019 is not applied as claimed.");
    process.exit(1);
  }
  console.log("\nDatabase is in the post-0019 shape with no google_ads_conversion_uploads table. OK.");
