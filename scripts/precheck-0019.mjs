...
// READ-ONLY pre-check for migration 0019 (CRM backup runs).
//
// Proves from live production evidence that 0019 is the ONLY migration this
...
const TAG = "0019_crm_backup_runs";
...
const allowed = [
  s => s.startsWith("CREATE TABLE `crm_backup_runs` (") && s.includes("CONSTRAINT `crm_backup_runs_id` PRIMARY KEY(`id`)"),
];
...
try {
  ...
  if (names.has("crm_backup_runs")) {
    console.error("\nSTOPPING: crm_backup_runs already exists.");
    process.exit(1);
  }
  ...
  console.log("\nDatabase is in the post-0016 shape with no crm_backup_runs table. OK.");
