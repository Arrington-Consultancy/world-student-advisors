...
// READ-ONLY pre-check for migration 0017 (Connector OAuth grants).
//
...
const TAG = "0017_connector_oauth_grants";
...
const allowed = [
  s => s.startsWith("CREATE TABLE `connector_oauth_grants` (") && s.includes("CONSTRAINT `connector_oauth_grants_id` PRIMARY KEY(`id`)"),
];
...
try {
  ...
  if (names.has("connector_oauth_grants")) {
    console.error("\nSTOPPING: connector_oauth_grants already exists.");
    process.exit(1);
  }
  ...
  console.log("\nDatabase is in the post-0016 shape with no connector_oauth_grants table. OK.");
