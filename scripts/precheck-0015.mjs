...
// READ-ONLY pre-check for migration 0015 (Routing Gap Log).
//
...
const TAG = "0015_routing_gap_log";
...
const allowed = [
  s => s.startsWith("CREATE TABLE `routing_gap_log` (") && s.includes("CONSTRAINT `routing_gap_log_id` PRIMARY KEY(`id`)"),
];
...
try {
  ...
  if (pending.length !== 1 || pending[0] !== TAG) {
    console.error(
      `\nSTOPPING: this run is authorised to apply migration 0015 and nothing else. ` +
        (pending.length === 0 ? "Nothing is pending; 0015 may already be applied." : `Pending set is [${pending.join(", ")}].`),
    );
    process.exit(1);
  }
  ...
  if (names.has("routing_gap_log")) {
    console.error("\nSTOPPING: routing_gap_log already exists.");
    process.exit(1);
  }
  // 0014 must be the real position, not just the journal's claim.
  const [cols] = await db.execute(sql`
    SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'staff_users' AND COLUMN_NAME = 'sessionVersion'
  `);
  if (cols.length === 0) {
    console.error("\nSTOPPING: staff_users.sessionVersion is missing, so 0014 is not applied as claimed.");
    process.exit(1);
  }
  console.log("\nDatabase is in the post-0014 shape with no routing_gap_log table. OK.");
