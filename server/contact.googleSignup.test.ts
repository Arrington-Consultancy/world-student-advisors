...
describe("6 – findGoogleUserForLogin honours the linked googleSub on subsequent login", () => {
  it("findGoogleUserForLogin is the lookup used by portal login after Google-linked registration", async () => {
    ...
    const result = await findGoogleUserForLogin(GOOGLE_PROFILE, true);
  ...
  it("does not create a new portal account when one already exists with that googleSub", async () => {
    // Simulate a returning Google user — findGoogleUserForLogin should return
    // the existing record, not insert a duplicate.
    mockedFindGoogleUserForLogin.mockResolvedValueOnce({
      status: "ok",
    ...
    expect(mockedFindGoogleUserForLogin).toHaveBeenCalledTimes(1);
    if (result.status === "ok") {
      expect(result.user.id).toBe(42);
    }
  });
